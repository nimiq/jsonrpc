...
async fn explicit_close_ends_streams_and_fails_pending_requests() {
    ...
    let url = serve(|mut ws| async move {
        ...
        // Report the second call, so that the test knows it is in flight before closing. It is
        // never answered, and neither is the close frame.
        let _ = next_request(&mut ws).await;
...
/// A notification the client can't parse must not kill the reader task: with the task gone,
/// the connection dying afterwards would never be noticed.
#[tokio::test(flavor = "multi_thread")]
async fn malformed_notification_does_not_kill_the_reader() {
    let url = serve(|mut ws| async move {
        ...
        ws.send(Message::text(malformed.to_string())).await.unwrap();

        // Delivering this proves the reader survived, then drop the TCP connection.
        notify(&mut ws, 1, json!(42)).await;
        drop(ws);
    })
    .await;
...
/// A subscriber that stops polling its stream fills up its channel and parks the reader task on
/// it. That must not block `close()`, which needs the same map the reader is delivering from.
#[tokio::test(flavor = "multi_thread")]
async fn close_returns_while_the_reader_is_stuck_on_a_full_stream() {
    ...
    // `close()` must return no matter what state the reader is in.
    assert_eq!(bounded(stream.next()).await, Some(0));
    bounded(sent_rx).await.unwrap();
