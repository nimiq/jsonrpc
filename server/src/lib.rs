...
impl<D: Dispatcher> Server<D> {
    ...
    async fn handle_single_request(
        ...
        if request.method == "rpc.methods" || request.method == "rpc.deprecatedMethods" {
            ...
            let methods = if request.method == "rpc.deprecatedMethods" {
                dispatcher.deprecated_methods()
            } else {
                dispatcher.method_names()
            };
            // Only respond if the request expects one (i.e. it isn't a notification).
            return request.id.map(|id| {
                let result =
                    serde_json::to_value(methods).expect("Failed to serialize method names");
...
pub trait Dispatcher: Send + Sync + 'static {
    ...
    /// Returns the names of the methods matched by this dispatcher that are marked `#[deprecated]`.
    ///
...
/// Logs a warning that a deprecated JSON-RPC method was called. This is invoked by dispatchers
/// generated by the `nimiq_jsonrpc_derive::service` macro and is exposed so that the generated code
/// doesn't have to depend on the `log` crate itself.
pub fn log_deprecated(method: &str, note: Option<&str>) {
    match note {
        Some(note) => log::warn!("Call to deprecated JSON-RPC method '{}': {}", method, note),
        None => log::warn!("Call to deprecated JSON-RPC method '{}'", method),
    }
}
